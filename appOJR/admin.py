...
class BarcoAdmin(admin.ModelAdmin):
    ...
    search_fields = ('id', 'nombre', 'armadora')
...
def enviar_solicitud_rancho(modeladmin, request, queryset):
    for rancho in queryset:
        ...
        # Solo marcamos como enviado si al menos un mail se mandó sin error
        if envio_exitoso:
...
class ContactoAgenciaAdmin(admin.ModelAdmin):
    ...
    search_fields = ('id', 'agenciaMaritima' ,	'correoElectronico' )
...
def generarDocumentosZip(idCarga):
    ...
    filename = str(carga.barco)+ str(carga.fechaInicio.day)+str(carga.fechaInicio.month)+str(carga.fechaInicio.year) + '.zip'
...
def generarRemito(remito):
    ...
    if (remito.carga.nroRancho is not None  ):
        nroRancho = "RANCHO " + remito.carga.fechaInicio.strftime("%y") + " " +  str(remito.carga.puerto.codigoAduana).zfill(3) 
...
def generarDocumentosDeseado(idCarga,cantidadCombustible,cantidadCamiones):
    ...
    context =  {'agenciaMaritima' : carga.barco.armadora.agenciaMaritima.nombre, 
                ...
                'vencimientoCertificadoInscripcion' : configuracion.vencimientoCertificadoInscripcion.strftime("%d/%m/%Y"),
                'fechaInicio' : carga.fechaInicio.strftime("%d/%m/%Y"),
                'horaI' : carga.horaInicio.strftime("%H:%M"),
                'horaF' :  (carga.horaInicio.strftime("%H:%M"))
    }
    doc.render(context)
    return doc
...
admin_site.register(Rancho, RanchoAdmin)  
admin_site.register(Barco, BarcoAdmin)
...
admin_site.register(ContactoAgencia, ContactoAgenciaAdmin)
